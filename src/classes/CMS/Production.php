...
class Production
{
    ...
    public function getAll($product_id = null, $order_id = null, $customer_name = null, $weave_by = null, $urgency = null, $written_by = null, $limit = 1000): array
    {
        $arguments['product_id']    = $product_id;           // Product ID
        $arguments['order_id']      = $order_id;             // Order ID
        $arguments['customer_name'] = $customer_name;        // Customer Name
        $arguments['weave_by']      = $weave_by;             // Weaver ID
        $arguments['urgency']       = $urgency;              // Urgency Level ID
        $arguments['written_by']    = $written_by;           // Admin ID
        $arguments['limit']         = $limit;                // Max articles to return


        $sql = "SELECT p.id, p.product_id, p.customer_name, p.created_at, pp.sku
                
                FROM production as p
                JOIN product as pp ON p.production_id = pp.id
                
                WHERE (p.product_id = :product_id OR :product_id is null)
                AND (p.order_id = :order_id OR :order_id is null)
                AND (p.customer_name = :customer_name OR :customer_name is null);";
